--- COPIED FROM https://github.com/Hammerspoon/hammerspoon/blob/master/extensions/window/switcher.lua
...
local function getSnapshot(id)
	...
	if not sn or sn[2] + SNAPSHOT_EXPIRY < now then
		snapshots[id] = { window.snapshotForID(id) or UNAVAILABLE, now }
	end
...
local function getIcon(bundle)
	...
	elseif not icons[bundle] then
		icons[bundle] = image.imageFromAppBundle(bundle) or UNAVAILABLE
	end
	return icons[bundle]
end
...
local function setFrames(nwindows, drawings, ui)
	...
	drawings.screenFrame = hs.mouse.getCurrentScreen():frame() --screen.mainScreen():frame()
	screenFrame = drawings.screenFrame
	...
	local bgframe =
		geom(0, 0, (size + padding) * nwindows + padding, size + padding + titleHeight + titlePadding):setcenter(
			screenFrame.center
		)
	...
	for i = 1, nwindows do
		local dr = drawings[i]
		local thumbFrame = geom(bgframe.x, bgframe.y, size, size):move(
			padding + (size + padding) * (i - 1),
			padding / 2 + titleHeight + titlePadding
		)
		...
		local selFrame = geom.copy(thumbFrame)
			:setw(selSize + selPadding * 2)
			:seth(selSize + selPadding * 2 + selTitleHeight)
			:setcenter(thumbFrame.center)
			:move(0, -selSize / 2 - size / 2 - padding * 1.5 - selPadding - selTitleHeight)
		if selFrame.x < screenFrame.x then
			selFrame.x = screenFrame.x
		elseif selFrame.x2 > screenFrame.x2 then
			selFrame.x = screenFrame.x2 - selFrame.w
		end
		...
		dr.highlightFrame = geom.copy(thumbFrame)
			...
			:setw(size + padding)
			:seth(size + padding + titleHeight + titlePadding)
...
local function draw(windows, drawings, ui)
	...
	if ui.showSelectedTitle then
		drawings.selTitleRect:show()
		drawings.selTitleText:show()
	end
	local haveThumbs, haveTitles = ui.showThumbnails, ui.showTitles
	...
	for i = 1, #windows do
		...
		if haveThumbs then
			dr.thumb:setImage(getSnapshot(win:id())):show()
		end
		...
		if haveTitles then
			...
			if titleWidth < titleFrame.w then
				titleFrame:setw(titleWidth):move((size - titleWidth) / 2, 0)
			end
...
local function exit(self)
	...
	if ui.showSelectedThumbnail then
		drawings.selThumb:hide()
		drawings.selIcon:hide()
	end
	if ui.showSelectedTitle then
		drawings.selTitleRect:hide()
		drawings.selTitleText:hide()
	end
	...
	end) -- el cap bugs out (desktop "floats" on top) if done directly
	--  end
end
...
local function modsPressed()
	-- Care about all BUT capslock
	local mods = checkMods()
	return mods.cmd or mods.alt or mods.shift or mods.ctrl or mods.fn
end
-- END KYLPO CHANGE
local function show(self, dir)
	...
	if not selected then -- fresh invocation, prep everything
		...
		if nwindows ~= drawings.lastn or drawings.screenFrame ~= hs.mouse.getCurrentScreen():frame() then -- they all must move
			setFrames(nwindows, drawings, ui)
			drawings.lastn = nwindows
		end
		self.drawDelayed = timer.doAfter(0.01, function()
			draw(windows, drawings, ui)
		end)
		self.modsTimer = timer.waitWhile(modsPressed, function()
			exit(self)
		end, MODS_INTERVAL)
		selected = 1
		self.log.df("activated, %d windows", nwindows)
	elseif self.drawDelayed:running() then
		self.drawDelayed:stop()
		draw(windows, drawings, ui)
	end
...
function switcher.nextWindow()
	return show(defaultSwitcher or makeDefault(), 1)
end
...
--- Notes:
---  * the switcher will be dismissed (and the selected window focused) when all modifier keys are released
function switcher.previousWindow()
	return show(defaultSwitcher or makeDefault(), -1)
end
...
local function setUiPrefs(self)
	...
	local selectedTitleTextStyle = {
		font = ui.fontName,
		size = math.floor(ui.textSize * 2),
		color = ui.textColor,
	...
	if ui.showSelectedTitle then
		...
		ui.selectedTitleHeight = 0
	end
	if ui.showSelectedThumbnail then
		ui.selectedPadding = ui.selectedThumbnailSize / 10
	else
		ui.selectedThumbnailSize, ui.selectedPadding = 0, 0
	end
	...
	if drawings.background then
		drawings.background:delete()
		drawings.highlightRect:delete()
		drawings.selRect:delete()
		drawings.selThumb:delete()
		drawings.selIcon:delete()
		drawings.selTitleRect:delete()
		drawings.selTitleText:delete()
	end
...
function switcher.new(wf, uiPrefs, logname, loglevel)
	...
	if type(uiPrefs) ~= "table" then
		error("uiPrefs must be a table", 2)
	end
	...
	self.ui = setmetatable({}, {
		__newindex = function(_, k, v)
			...
			return not inUiPrefs and setUiPrefs(self)
		end,
		__index = function(_, k)
			return getColor(uiGlobal[k])
		end,
	})
	for k, v in pairs(uiPrefs) do
		rawset(self.ui, k, getColor(v))
	end
	setUiPrefs(self)
