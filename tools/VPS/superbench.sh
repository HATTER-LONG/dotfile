...
benchinit() {
	...
	# worsttrace
	#if [ ! -e './worsttrace' ]; then
	#	echo " Installing WorstTrace..."
	#	wget --no-check-certificate -T 10 -qO worsttrace https://down.vpsaff.net/linux/worsttrace/worsttrace > /dev/null 2>&1
	#fi
	#chmod +x ./worsttrace
	...
	#if [ ! -e './nexttrace' ]; then
	#	if [[ "$ARCH" == "x64" ]]; then
	#		wget --no-check-certificate -T 10 -qO nexttrace https://down.vpsaff.net/linux/nexttrace/nexttrace_linux_amd64 > /dev/null 2>&1
	#	elif [[ "$ARCH" == "x86" ]]; then
	#		wget --no-check-certificate -T 10 -qO nexttrace https://down.vpsaff.net/linux/nexttrace/nexttrace_linux_386 > /dev/null 2>&1
	#	elif [[ "$ARCH" == "aarch64" ]]; then
	#		wget --no-check-certificate -T 10 -qO nexttrace https://down.vpsaff.net/linux/nexttrace/nexttrace_linux_amd64 > /dev/null 2>&1
	#	elif [[ "$ARCH" == "arm" ]]; then
	#		wget --no-check-certificate -T 10 -qO nexttrace https://down.vpsaff.net/linux/nexttrace/nexttrace_linux_armv7 > /dev/null 2>&1
	#	fi
	#fi
	#chmod +x ./nexttrace
...
traceroute_test() {
	...
	#WorstTrace
	#echo -e "\n worsttrace to $4 (ICMP Mode, Max $3 Hop)" | tee -a $log
    #echo -e " =========================================================================================" | tee -a $log
    #./worsttrace -n --maxhop $3 $1 | tail -n +5 | tee -a $log
	
	#NextTrace
	#if [ "$2" = "tcp" ] || [ "$2" = "TCP" ]; then
    #    echo -e "\nNextTrace to $4 (TCP Mode, Max $3 Hop)" | tee -a $log
    #    echo -e "============================================================" | tee -a $log
    #    ./nexttrace $1 -m $3 -g en -T -q 1 2>&1 | tail -n +4 | head -n -1 | tee -a $log
    #else
    #    echo -e "\nNextTrace to $4 (ICMP Mode, Max $3 Hop)" | tee -a $log
    #    echo -e "============================================================" | tee -a $log
    #    ./nexttrace $1 -m $3 -g en -q 1 2>&1 | tail -n +4 | head -n -1 | tee -a $log
    #fi
}

print_traceroute_test(){
	#echo -e " WorstTrace 2.0.8"
	#echo -e " Join Our group at: https://t.me/WorstTrace"

	traceroute_test "113.108.209.1" "TCP" "30" "China, Guangzhou CT"
	traceroute_test "180.153.28.5" "TCP" "30" "China, Shanghai CT"
    traceroute_test "180.149.128.9" "TCP" "30" "China, Beijing CT"
	traceroute_test "210.21.4.130" "TCP" "30" "China, Guangzhou CU"
	traceroute_test "58.247.8.158" "TCP" "30" "China, Shanghai CU"
	traceroute_test "123.125.99.1" "TCP" "30" "China, Beijing CU"
	traceroute_test "120.196.212.25" "TCP" "30" "China, Guangzhou CM"
    traceroute_test "221.183.55.22" "TCP" "30" "China, Shanghai CM"
	traceroute_test "211.136.25.153" "TCP" "30" "China, Beijing CM"
	traceroute_test "211.167.230.100" "TCP"  "30" "China, Beijing Dr.Peng Network IDC Network"	
}
...
function YouTubeCDNTest() {
	...
	local IATACode=$(curl -s --retry 3 --max-time 10 "https://down.vpsaff.net/iata/IATACode.txt");
    ...
    if [ -n "$iata" ] && [ -z "$isIDC" ]; then
		...
		echo -e " YouTube CDN          : ${YELLOW}$CDN_ISP in $location${PLAIN}" | tee -a $log
        return
